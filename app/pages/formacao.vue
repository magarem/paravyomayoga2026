...
<div class="absolute -top-[300px] md:-top-[550px] left-1/2 -translate-x-1/2 z-0 pointer-events-none opacity-[0.04]">
    ...
    <div class="container mx-auto px-4 max-w-6xl">
      ...
      <section class="mb-7 text-center">
        ...
        <h1 class="text-5xl md:text-7xl font-serif text-[#4a3728] mb-6 leading-tight">
          {{ content.title }}
        </h1>
  ...
  <div class="lg:col-span-5 flex flex-col gap-3">
    ...
    <div class="bg-amber-50 p-8 rounded-3xl border-2 border-amber-200/50 shadow-inner">
      ...
      <div class="grid grid-cols-1 gap-3">
        <div v-for="extra in content.a_quem_se_destina" :key="extra" 
          class="flex items-center bg-white border border-amber-200 px-4 py-3 rounded-xl shadow-sm transition-transform hover:translate-x-2">
          <i class="pi pi-users text-[#4a3728] mr-3 font-bold"></i>
          <span class="text-[#2d241e] font-bold text-base">{{ extra }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="p-7 bg-white border-2 border-[#25D366]/30 rounded-[2rem] shadow-lg flex flex-col items-center text-center">
      ...
      <section class="bg-white p-10 rounded-[3rem] border-2 border-[#4a3728]/10 shadow-lg">
        <div class="flex flex-col md:flex-row gap-12 items-center">
          <div class="relative">
            <div class="absolute inset-0 bg-[#4a3728] rounded-full translate-x-2 translate-y-2"></div>
            <Avatar image="/images/paravyoma.png" size="xlarge" shape="circle" class="w-48 h-48 border-4 border-white relative z-10" />
          </div>
          <div>
            <h2 class="text-4xl font-serif text-[#4a3728] mb-6">Seu Instrutor: {{ content.instrutor.nome }}</h2>
